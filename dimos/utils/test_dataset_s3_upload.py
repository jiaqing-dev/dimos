...
def _set_repo_root(monkeypatch, repo_root: Path) -> None:
    monkeypatch.setattr(data_utils, "_get_repo_root", lambda: repo_root)
...
def test_pack_dataset_rejects_path_traversal(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="must not contain"):
        pack_dataset_tar_gz("../outside", tmp_path / "out.tar.gz")


def test_pack_dataset_rejects_dataset_root_symlink_escape(
    tmp_path,
    monkeypatch,
) -> None:
    _set_repo_root(monkeypatch, tmp_path)
