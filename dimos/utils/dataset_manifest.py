...
def validate_dataset_name(dataset_name: str) -> str:
    """Return a safe single-directory dataset name or raise ``ValueError``."""
    if not _DATASET_NAME_RE.fullmatch(dataset_name):
        raise ValueError(
            "Dataset name must be a non-empty single path segment containing only "
...
def dataset_dir_path(dataset_name: str) -> Path:
    """Resolve ``data/<dataset_name>`` and require it to stay under ``data/``."""
    safe_name = validate_dataset_name(dataset_name)
    data_root = get_data_dir().resolve()
    dataset_root = (data_root / safe_name).resolve()
    try:
...
def build_go2_manifest_payload(
    ...
    *,
    ...
    root = dataset_dir_path(dataset_name)
